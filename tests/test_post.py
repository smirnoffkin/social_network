...
@pytest.mark.parametrize("email, post_id, reaction", [
    ("user@example.com", 2, PostReaction.LIKE),
    ("pepe@example.com", 1, PostReaction.LIKE),
    ("pepe@example.com", 3, PostReaction.LIKE),
    ("google@example.com", 2, PostReaction.DISLIKE),
])
...
@pytest.mark.parametrize("email, post_id, reaction", [
    ("user@example.com", 2, PostReaction.DISLIKE),
    ("pepe@example.com", 1, PostReaction.DISLIKE),
    ("pepe@example.com", 3, PostReaction.DISLIKE),
    ("google@example.com", 2, PostReaction.LIKE),
])
...
async def test_add_reaction_to_own_post(client: AsyncClient, email: str, post_id: int, reaction: PostReaction):
    headers = await create_test_auth_headers_for_user(email)
    res = await client.post(
        url=f"/post/{id}/reaction/{reaction.value}?post_id={post_id}",
        headers=headers
    ...
    assert res.status_code == status.HTTP_403_FORBIDDEN
    assert res.json() == {"detail": "You can't react to your own posts."}
